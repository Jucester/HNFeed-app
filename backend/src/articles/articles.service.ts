...
export class ArticlesService {
    ...
    constructor(private http: HttpService, @InjectModel('Articles') private readonly articlesModel : Model<IArticle> ) {}
    ...
    async savingArticles() : Promise<Observable<AxiosResponse<any>>> {
        ...
        return await this.http.get('http://hn.algolia.com/api/v1/search_by_date?query=nodejs')
                              .toPromise()
                              .then(res => {
                                    ...
                                    res.data.hits.forEach( async hit => {
                                        if(hit.title || hit.story_title) {
                                            ...
                                            const prev = await this.articlesModel.findOne({story_id: hit.story_id});                      
